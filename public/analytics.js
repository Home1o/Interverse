...
(function (global) {
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      ".iv-an-said{font-size:12.5px;line-height:1.65;color:var(--mut,#66707D);margin-top:8px;padding-left:11px;border-left:2px solid var(--line,#D9DEE6)}",
      ".iv-an-t{background:rgba(214,69,61,.10);color:var(--ink,#1F2A37);border-bottom:1.5px solid rgba(214,69,61,.55);border-radius:2px;padding:1px 2px;box-decoration-break:clone;-webkit-box-decoration-break:clone}",
  ...
  /* A segment runs from its own offset to the start of the next located one.
     If the offsets came back out of order the model lost the plot, so the
     spans are dropped rather than shown wrong. */
  function attachText(rec) {
    var src = rec.a || "", segs = rec.segments || [];
    var known = segs.filter(function (g) { return g.at >= 0; });
    for (var i = 1; i < known.length; i++) {
      if (known[i].at <= known[i - 1].at) { segs.forEach(function (g) { g.text = ""; }); return; }
    }
    if (known.length && known[0].at > 0) known[0].at = 0;

    // A quote marks where a segment starts, so the words leading into the next
    // quote would otherwise be swallowed by the previous segment. Pull each
    // boundary back to the sentence end before it.
    var prev = 0;
    known.forEach(function (g, i) {
      if (i === 0) { g.at = 0; prev = 0; return; }
      var k = -1;
      for (var c = g.at - 1; c > prev; c--) {
        if (src[c] === "." || src[c] === "!" || src[c] === "?") { k = c; break; }
      }
      if (k > prev) g.at = k + 1;
      prev = g.at;
    });
    ...
    segs.forEach(function (g, i) {
      if (g.at < 0) { g.text = ""; return; }
      var end = src.length;
      for (var j = i + 1; j < segs.length; j++) if (segs[j].at >= 0) { end = segs[j].at; break; }
      g.text = src.slice(g.at, end).trim();
    });
  }

  /* The answer as alternating plain / tangent runs, shortened in the middle
     of long plain stretches so one rambling answer cannot fill the report. */
  function spans(rec) {
    var segs = (rec.segments || []).filter(function (g) { return g.text; });
    if (!segs.length) return null;
    if (!segs.some(function (g) { return g.role === "tangent"; })) return null;
    var out = [], buf = "";
    segs.forEach(function (g) {
      if (g.role === "tangent") {
        if (buf) { out.push({ t: buf, x: false }); buf = ""; }
        out.push({ t: g.text, x: true });
      } else buf += (buf ? " " : "") + g.text;
    });
    if (buf) out.push({ t: buf, x: false });
    var total = out.reduce(function (n, s) { return n + s.t.length; }, 0);
    if (total > 560) out.forEach(function (s) {
      if (!s.x && s.t.length > 150) s.t = s.t.slice(0, 70).trim() + " \u2026 " + s.t.slice(-70).trim();
    });
    return out;
  }

  /* Same guard the live tips use: a quote is only trusted if the candidate
     actually said it. Models paraphrase under pressure.
     normMap also returns an index back to the original string, so a verified
     quote can be turned into a real character offset in the transcript. */
  function norm(t) { return String(t || "").toLowerCase().replace(/[^a-z0-9 ]+/g, " ").replace(/\s+/g, " ").trim(); }

  function normMap(src) {
    var out = "", idx = [], lastSpace = true;
    for (var i = 0; i < src.length; i++) {
      var c = src[i].toLowerCase();
      if (c >= "a" && c <= "z" || c >= "0" && c <= "9") { out += c; idx.push(i); lastSpace = false; }
      else if (!lastSpace) { out += " "; idx.push(i); lastSpace = true; }
    }
    while (out.length && out[out.length - 1] === " ") { out = out.slice(0, -1); idx.pop(); }
    return { text: out, idx: idx };
  }

  /* -> character offset in source, or -1 if the model made the quote up */
  function locate(quote, source) {
    var nq = norm(quote);
    if (!nq || nq.split(" ").length < 3) return -1;
    var m = normMap(source);
    var at = m.text.indexOf(nq);
    return at === -1 ? -1 : m.idx[at];
  }
  ...
  function reportHtml() {
    ...
    var driftRows = flagged.map(function (f) {
      ...
      var sp = spans(f.a);
      if (sp) {
        h += '<div class="iv-an-said">' + sp.map(function (x) {
          return x.x ? '<mark class="iv-an-t">' + esc(x.t) + "</mark>" : esc(x.t);
        }).join(" ") + "</div>";
      }
      return h + "</div>";
    }).join("");
    ...
    return head +
      '<div class="card" style="padding:20px;margin-bottom:16px">' +
        '<div class="eyebrow" style="margin-bottom:4px">WHERE YOU DRIFTED</div>' +
        '<div class="iv-an-note" style="margin-bottom:14px">Highlighted text is where the answer left the question.</div>' + body +
      "</div>";
  }
