...
async function handleMaterialFile(input) {
  ...
  try {
    ...
    input.value = "";
    render();
    if (r.truncated) toast("Long file \u2014 first 60,000 characters loaded");
...
function goNewSession() {
  ...
  S.scenario = ""; S.entry = "material"; S.docName = ""; S.docKind = ""; S.docSlides = 0;
...
function onMaterialInput(el) {
  ...
  S.docName = "";
  var b = $("su-entry");
  if (b) b.style.display = S.content.trim() ? "block" : "none";
}

function entryHtml() {
  var on = !!(S.content || "").trim();
...
function setupHtml() {
  var docStatus = S.docName
    ...
    : 'PPTX, DOCX, PDF or TXT \u2014 slides and speaker notes are read.';
  ...
  return '<div class="eyebrow" style="margin-bottom:10px">1 · CHOOSE YOUR MODE</div>' +
    ...
    '<div class="filerow">' +
    '<label class="btn btn-ghost filebtn">\ud83d\udcce Upload deck or document' +
    '<input type="file" accept=".pptx,.docx,.pdf,.txt,.md,.csv" onchange="handleMaterialFile(this)"/></label>' +
