...
var S = {
  ...
  recording: false,       // MediaRecorder is capturing this answer
  transcribing: false,    // waiting on Whisper before the answer is submitted
...
/* Which screen is actually IN FRONT of the person — not what S.screen happens
   to hold. S.screen defaults to "setup" from the moment the app loads, so
   reporting it directly logged a phantom "setup" for every visitor who only
   ever saw the landing page, and made the funnel look like everyone reached
   the practice room. The DOM is the only honest source here. */
function shown(id) {
  var el = $(id);
  return !!el && el.style.display !== "none" && el.offsetParent !== null;
}

/* render() reports the screen on every app redraw, but the auth screens swap
   panes by touching display directly and never call it — so without this the
   funnel silently lost the "opened sign-in" and OTP steps. */
function trackScreen() {
  if (window.IVTrack) IVTrack.screen(visibleScreen());
}

function visibleScreen() {
  if (shown("auth-screen")) {
    var panes = { "pane-otp": "otp", "pane-forgot": "forgot", "pane-reset": "reset" };
    for (var id in panes) if (shown(id)) return panes[id];
    return "auth";
  }
  if (shown("app-shell")) return S.screen;
  return "landing";
}
...
/* ============================================================
   ...
   before the stylesheet loads, so a dark-mode user never sees a white flash.
   This part only handles switching it and keeping the two toggles in sync.
   ============================================================ */
...
function applyTheme(theme) {
  ...
  var t = $("theme-toggle"), icon = $("theme-icon"), label = $("theme-label"), lpIcon = $("lp-theme-icon");
  if (icon) icon.textContent = dark ? "☀" : "☾";
  if (label) label.textContent = dark ? "Light mode" : "Dark mode";
  if (t) t.setAttribute("aria-pressed", dark ? "true" : "false");
  if (lpIcon) lpIcon.textContent = dark ? "☀" : "☾";

  // The browser chrome (mobile address bar) should match the page behind it.
  var meta = document.querySelector('meta[name="theme-color"]');
  if (meta) meta.setAttribute("content", dark ? "#10161D" : "#1B2430");
}
...
/* Follow the OS while the person hasn't expressed a preference of their own. */
(function () {
  if (!window.matchMedia) return;
  var mq = window.matchMedia("(prefers-color-scheme: dark)");
  var onChange = function () {
    var saved = null;
    try { saved = localStorage.getItem("iv_theme"); } catch (e) {}
...
async function startListening() {
  ...
  /* The recorder is the source of truth; recognition above only paints the live
     caption. Speech onset comes from the audio level rather than from a
     recognition result, so the pause measurement survives a browser where Web
     Speech never fires. */
  if (window.IVRec && IVRec.supported())
    IVRec.start({ onSpeechStart: markSpeechStarted }).then(function (ok) {
      S.recording = !!ok;
    });
}
function stopListening(send) {
  ...
  var caption = (finalText + " " + interimText).trim();
  finalText = ""; interimText = ""; S.interim = "";

  if (!send) {
    if (window.IVRec) IVRec.stop();
    S.recording = false; S.phase = "idle"; render();
    return;
  }
  finishAnswer(caption);
}

/* The recorded clip is transcribed by Whisper and that text is what gets
   scored. The live caption is only a fallback for when there is no recording,
   the upload fails, or the service is rate limited — in those cases a partial
   answer still beats losing it. */
async function finishAnswer(caption) {
  if (!window.IVRec || !S.recording) {
    S.recording = false;
    if (caption) submitAnswer(caption);
  ...
  var text = "";
  try {
    var clip = await IVRec.stop();
    // Nothing captured, or the whole clip sat below speaking level: treat it as
    // silence rather than sending it to a model that will invent words.
    if (clip && clip.blob && clip.peak >= IVRec.SPEECH_RMS) {
      var dataUrl = await IVRec.blobToDataUrl(clip.blob);
      var r = await api("/transcribe", "POST", {
        audio: dataUrl,
        mime: clip.mime,
    ...
    if (/rate limited/i.test(e.message || "")) toast("Transcription is busy — using the live caption");
  }

  S.transcribing = false;
  var final = text || caption;
  if (final) submitAnswer(final);
  else {
    S.phase = "idle";
    toast("Didn't catch that — try again, or type your answer");
    render();
  }
}

/* Whisper takes a prompt for context. Feeding it the proper nouns from the
   candidate's own material is why their name, their company and their project
   jargon come back spelled rather than guessed at phonetically. */
function transcriptionHint() {
  var bits = [];
  var p = S.profile || {};
  if (p.name) bits.push(p.name);
  if (S.role) bits.push(S.role);
  var source = ((S.content || "") + " " + (p.resume || "")).slice(0, 6000);
  // Capitalised words and acronyms are the ones speech models get wrong.
  var caps = source.match(/\b[A-Z][A-Za-z0-9&.\-]{2,}(?:\s+[A-Z][A-Za-z0-9&.\-]{2,})?\b/g) || [];
  var seen = {}, picked = [];
  for (var i = 0; i < caps.length && picked.length < 40; i++) {
    var w = caps[i].trim();
    if (w.length < 3 || seen[w.toLowerCase()]) continue;
    seen[w.toLowerCase()] = 1;
    picked.push(w);
  }
  return bits.concat(picked).join(", ").slice(0, 600);
}
...
function markSpeechStarted(at) {
  if (S.thinkMs !== null || !S.askedAt) return;
  // `at` is the moment the audio level showed speech beginning. Web Speech has
  // no such timestamp, so it passes nothing and we use now.
  var began = typeof at === "number" && at > S.askedAt ? at : Date.now();
  S.thinkMs = began - S.askedAt;
  stopThinkClock();
  S.spokeFrom = began;        // the speaking clock starts at the first real word too
}
...
"5a. THE TRANSCRIPT IS MACHINE SPEECH RECOGNITION, not typing. An odd or wrong-sounding single word is usually a mis-hearing, not the candidate's vocabulary — never correct their word choice, grammar or spelling on the strength of one strange word, and never quote a garbled fragment back at them as if they said it. Judge what they clearly meant. Only flag wording when a whole phrase is consistently weak.\n" +
...
function sessionHtml() {
  ...
  var orbLabel = S.phase === "listening" ? "Tap to send your answer"
    : S.transcribing ? "Reading back what you said…"
...
function boot() {
  ...
  applyTheme(currentTheme());   // labels/icons, not the attribute — that's already set
  if (window.IVTrack && user && user.id) IVTrack.identify(user.id);
  restoreSidebar();
  if (token && user) {
    $("landing").style.display = "none";
    $("auth-screen").style.display = "none";
