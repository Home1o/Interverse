...
async function doForgotStart() {
  var email = (forgotEmail || $("fp-email").value || "").trim();
...
function systemPrompt() {
  ...
  var base =
"You are Interverse, a live spoken-voice practice partner. Your reply is SPOKEN ALOUD.\n\n" +
"== CANDIDATE'S MATERIAL ==\n" + material + "\n\n" +
"== CANDIDATE'S TARGET GOAL ==\n" + goal + "\n\n" +
"== HARD RULES (never break these) ==\n" +
"1. TOPIC LOCK: every question you ask must be traceable to (a) the material above, (b) the target goal, or (c) something the candidate said in a previous answer. Never introduce unrelated topics, trivia, or generic small talk. If material and goal are both missing, pick ONE professional theme in your first question and stay on it for the whole session.\n" +
"2. ONE question per turn. Under 80 words spoken. Natural conversational English — no markdown, no bullets, no headers, no emojis in the spoken reply.\n" +
"3. DEPTH BEFORE BREADTH: ask at least one follow-up on the candidate's answer before moving to a new topic from the material. Never repeat a question already asked.\n" +
"4. Do not answer for the candidate, do not lecture, do not summarize the material back to them.\n" +
"5. OUTPUT FORMAT: respond with ONLY this JSON object — no code fences, no text before or after:\n" +
'{"reply": "<what you say aloud>", "tip": "<on-screen coaching note per your mode rules, or empty string>"}\n\n';

  if (S.mode === "interview")
    return base +
"== YOUR MODE: INTERVIEW DRILL ==\n" +
"Objective: a realistic, rigorous interview (MBA admissions / senior hiring panel) on the candidate's OWN material.\n" +
"- First turn: one-line welcome, then a question grounded in a specific item from the material.\n" +
"- Probe every answer on the SAME story before moving on: ask them to quantify impact, justify a decision, name a trade-off, or say what they'd do differently.\n" +
"- Escalate difficulty as the session progresses: start factual, move to 'why you', end with stress questions (weaknesses, failures, conflicting priorities) — still tied to their material.\n" +
"- tip is MANDATORY after every candidate answer (max 22 words): one live note on answer QUALITY — missing structure (situation/action/result), vague claims that need a number, or dodging — quoting a fragment of their words when useful. If the answer was genuinely strong, name its strongest element in one line. First turn only: tip is an empty string.";

  if (S.mode === "confidence")
    return base +
"== YOUR MODE: CONFIDENCE COACH ==\n" +
"Objective: train assertive, decisive DELIVERY. Question topics still come from the material/goal (rule 1), but your analysis targets HOW they speak, not what they know.\n" +
"- Ask challenging behavioural questions drawn from their material: ownership, conflict, decisions under pressure.\n" +
"- tip is MANDATORY after every candidate answer, max 30 words, exactly this job: QUOTE one weak fragment from their answer (hedging like 'I think maybe', passive voice, apologising, qualifiers, filler) and give the assertive rewrite. Format: '\u201c<their words>\u201d \u2192 \u201c<assertive version>\u201d'. If their delivery was genuinely strong, say which sentence was strongest and why in one line.\n" +
"- Your spoken reply never discusses delivery — that lives only in tip. Spoken reply = brief acknowledgement + next question.";

  return base +
"== YOUR MODE: VOCABULARY BUILDER ==\n" +
"Objective: upgrade the candidate's word choice and phrasing. Question topics come from the material/goal (rule 1) and should invite rich description, opinion, and storytelling about it.\n" +
"- tip is MANDATORY after every candidate answer: pick 2-3 words or phrases THEY ACTUALLY USED and upgrade them. Format each as: <their word> \u2192 <stronger word> (micro-gloss). Prefer upgrades useful in interviews and professional settings.\n" +
"- In your next spoken question, naturally reuse ONE of the upgraded words so they hear it in context.\n" +
"- Never invent quotes — only upgrade words that appear in their answer.";
}
...
/* Per-turn reinforcement: smaller models drift from the system prompt as the
   conversation grows. We append a compact reminder to the LAST user message
   at request time only — S.history itself stays clean. */
function turnReminder() {
  var qNum = S.turns.filter(function (t) { return t.who === "ai"; }).length + 1;
  var modeBit = S.mode === "interview"
    ? "tip is REQUIRED: one short note on my answer's structure, specificity, or dodging — quote my words. Then probe my last answer or move to the next item FROM MY MATERIAL."
    : S.mode === "confidence"
      ? "tip is REQUIRED: quote one weak fragment from my answer and rewrite it assertively."
      : "tip is REQUIRED: 2-3 words I actually used, upgraded as weak \u2192 strong.";
  return "\n\n[Reminder — do not mention this bracket: You are in " + MODES[S.mode].label.toUpperCase() +
    " mode, asking question " + qNum + ". Stay locked to my material/goal or my answers — no new topics. " +
    modeBit + " Respond with ONLY the JSON object {\"reply\",\"tip\"}.]";
}
...
async function aiTurn(userText) {
  ...
  try {
    ...
    // clone history and reinforce the last user message at send time only
...
async function endSession() {
  ...
  var sys = 'You are an expert speaking coach. Analyze the interview transcript. Respond with ONLY JSON, no fences:\n' +
    ...
    '- power_phrases: 4-6 items, MANDATORY. Each "weak" must be an EXACT word or short phrase copied from the CANDIDATE lines — weak verbs (did, got, helped, worked on), vague nouns (stuff, things, a lot), hedges (I think, maybe, kind of), and filler. Each "strong" is the professional upgrade they should say instead. Never invent quotes.\n' +
