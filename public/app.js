...
var MODES = {
  ...
  hrbatch: { label: "HR Question Practice", persona: "Batch", tag: "HR SET", color: "#8A4FFF",
    blurb: "Practice a set of 5 or 10 HR questions tailored to your profile. See the whole list up front, then answer one by one." },
  ...
  extempore: { label: "Extempore", persona: "Rostrum", tag: "EXTEMPORE", color: "#1F5FA8",
    blurb: "Rostrum gives you a topic and you speak on it for a minute or two, unprepared. Judged on how well you hold the floor." }
};
...
/* Extempore is judged partly on holding the floor for a minute or two, so the
   speaker needs to see the time. Written straight into the DOM rather than
   through render(), which would tear down the live transcript every second. */
var speakTimer = null;
...
"- FIRST TURN ONLY (may run to 90 words): introduce yourself by name and preview the focus, e.g. \u201cHey, I'm Drill. Today I'll be interviewing you on <one-line summary of their material or goal — name the actual topic>.\u201d " + openLine + " Never repeat this introduction later.\n" +
...
"- tip is MANDATORY every turn (max 30 words): one concrete coaching note on their previous answer \u2014 structure (did they use situation/action/result?), a vague claim needing a number, or a stronger way to phrase it. Empty on the very first turn.";
  ...
  if (S.mode === "extempore") {
    ...
    return base + basis +
"== YOUR MODE: EXTEMPORE ==\n" +
"Your name is Rostrum. You run impromptu speaking practice. You set a topic, they speak on it unprepared for one to two minutes, and you judge what they said. You are a speaking coach, not an interviewer — you do not interrogate, you evaluate.\n" +
"- FIRST TURN ONLY (may run to 90 words): introduce yourself briefly — \u201cHi, I'm Rostrum. I'll give you a topic, you speak on it for a minute or two, and I'll tell you how it landed.\u201d Tell them to take a few seconds to think before they start. Then set the FIRST TOPIC, clearly and slowly, and STOP. tip is an empty string on this turn.\n" +
...
"- FIRST TURN ONLY (may run to 90 words): introduce yourself, e.g. \u201cHi, I'm Lexicon. Today we'll have a conversation about <their material/goal in one line>, and I'll help you find sharper words as we go.\u201d " + openLine + " Never repeat this introduction later.\n" +
...
function goNewSession() {
  ...
  S.topicSrc = "generic"; S.topicKind = "general";
  S.content = ""; S.role = "";
...
function setupHtml() {
  ...
  if (S.mode === "extempore") {
    ...
    topicBit = '<div class="card" style="padding:16px 18px;margin-bottom:24px">' +
      ...
      (S.topicSrc === "material"
        ? "Rostrum sets topics from your material and judges each speech on how faithfully you used it \u2014 accuracy, its specifics, and whether you added a view of your own."
        : "Rostrum sets an open topic and judges the thinking you convey \u2014 clear position, development, evidence, and whether you land it. Nothing to upload; you can start now.") +
      '</div></div>';
  }
  ...
  var matHeading = S.mode === "extempore" && S.topicSrc === "generic"
    ...
    : "2 · FEED YOUR MATERIAL";
  return '<div class="eyebrow" style="margin-bottom:10px">1 · CHOOSE YOUR MODE</div>' +
    '<div class="grid3">' + cards + '</div>' +
    batchBit + topicBit +
    '<div class="eyebrow" style="margin-bottom:10px">' + matHeading + '</div>' +
    ...
    entryHtml() + '</div>' +
    '<div class="eyebrow" style="margin-bottom:10px">3 · SET THE SCENE <span style="font-weight:400;text-transform:none;letter-spacing:0">(optional)</span></div>' +
    '<div class="card" style="padding:18px;margin-bottom:24px">' +
    ...
    '<div class="eyebrow" style="margin-bottom:10px">4 · GO LIVE</div>' +
