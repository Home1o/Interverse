...
var PROMPT_CHARS = {
  material: 6000,     // ~1,650 tokens
  resume: 1800,       // ~500
  transcript: 9000,   // end-of-session report: one request, nothing running beside it
  historyTurns: 12,   // most recent messages kept
  historyMsg: 1500,   // per message
  historyTotal: 6000  // ~1,650 tokens across the lot
};
...
async function aiTurn(userText) {
  ...
  try {
    ...
    var req = { system: systemPrompt(), messages: payload, max_tokens: 1000 };
    if (S.image) req.images = [S.image];
    var r = await api("/chat", "POST", req);
    ...
    while (!done) {
      try {
        var r = await api("/chat", "POST", {
          ...
          images: batch,
          max_tokens: 2500,
          messages: [{ role: "user", content: "Transcribe " + label + " " + (i + 1) +
            (batch.length > 1 ? " and " + (i + 2) : "") + " of " + images.length + "." }]
        });
        ...
        var wait = /rate limited/i.test(e.message) ? 20000 : 0;
        if (wait && attempt === 0) {
          attempt++;
          if (onProgress) onProgress(i + 1, images.length, "waiting out the rate limit\u2026");
...
function boot() {
  ...
  if (window.IVAnalytics) IVAnalytics.init({ apiFn: api, onUpdate: render });
  restoreSidebar();
  if (token && user) {
    $("landing").style.display = "none";
    $("auth-screen").style.display = "none";
