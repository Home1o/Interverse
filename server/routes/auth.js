...
async function issueOtp(email) {
  ...
  // Always visible in server logs IMMEDIATELY — before any DB/mail latency.
  console.log(`[otp] code for ${email}: ${code} (valid 10 min)`);
  await client.execute({
    sql: "INSERT INTO otps (email, code, expires_at) VALUES (?, ?, datetime('now','+10 minutes'))",
    args: [email, code],
  });
  // Fire-and-forget: the API responds instantly; email sends in the background.
  sendOtp(email, code).catch((e) =>
    console.error("[mail] send failed:", e.message, "| OTP for", email, "is", code)
  );
  return code;
}
...
router.post("/register", async (req, res) => {
  try {
    ...
    if (existing.rows.length) {
      ...
      // unverified re-register: refresh details + resend code, and say so clearly
      await client.execute({
      ...
      return res.json({
        ok: true, needVerify: true, code: "EMAIL_EXISTS_UNVERIFIED",
        message: "This email was already registered but never verified — we've sent a fresh code to " + email,
        ...(DEV ? { devOtp: code } : {}),
      });
    }
    await client.execute({
