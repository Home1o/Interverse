...
const SCHEMA = [
  ...
  /* One row per visit, updated in place rather than an event log: it answers
     both "how many times have they been here" and "where were they when they
     left" with a single write per screen change, which matters on a free tier
     metered by row writes.

     `path` is the whole journey as one breadcrumb string
     ("landing>auth>profile>setup>session"), so the last element IS the exit
     point and the whole column is a funnel. No answer content, no IP, no full
     user agent — only where they went. */
  ...
  `CREATE INDEX IF NOT EXISTS idx_visits_visitor ON visits(visitor_id, started_at DESC)`,
